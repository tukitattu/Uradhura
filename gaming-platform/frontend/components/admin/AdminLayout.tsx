...
import {
  ...
  FileText, Shield, Settings, LogOut, Bell, Search,
} from 'lucide-react';
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(()=>{
    if (!loading&&(!player||(player.role!=='admin'&&player.role!=='super_admin'))) router.push('/login');
  ...
  return (
    <div className="flex min-h-screen" style={{background:'#0a0010'}}>
      {/* Sidebar */}
      <aside className="dl-sidebar w-52 flex flex-col flex-shrink-0 fixed h-full z-20">
        <div className="p-4 border-b border-[rgba(61,17,85,0.6)]">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <div className="text-[9px] text-[rgba(255,255,255,0.35)] uppercase tracking-wide">Operations Panel</div>
            </div>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              {player.role==='super_admin'&&(
                <span className="text-[9px] font-black px-2 py-0.5 rounded-full bg-gradient-to-r from-[#ff1fa6] to-[#8b00ff] text-white uppercase tracking-wide">Super Admin</span>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="relative hidden md:block">
              ...
              <input value={search} onChange={e=>setSearch(e.target.value)}
                onKeyDown={e=>{ if(e.key==='Enter'&&search.trim()){router.push(`/admin/players?search=${encodeURIComponent(search)}`);setSearch('');} }}
            ...
            <div className="flex items-center gap-2 cursor-pointer group" onClick={logout}>
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#ff1fa6] to-[#8b00ff] flex items-center justify-center text-white font-black text-sm glow-pink">
                {player.username[0]}
              </div>
