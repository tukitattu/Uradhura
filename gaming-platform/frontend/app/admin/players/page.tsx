    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr key={p.id} onClick={() => handleSelectPlayer(p)}
                      className="border-b border-[rgba(61,17,85,0.6)]/50 hover:bg-white/5 cursor-pointer transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <div className="w-7 h-7 rounded-full bg-[#ff1fa6]/20 flex items-center justify-center text-[#ff1fa6] text-xs font-bold">{p.username[0]}</div>
                          <div>
                      ...
                      <td className="px-4 py-3 text-sm font-bold text-[#ffd700]">🪙 {formatTokens(p.balance)}</td>
                      <td className="px-4 py-3 text-sm text-[#00e676]">+{formatTokens(p.totalWon)}</td>
                      <td className="px-4 py-3 text-sm text-[#ff3d57]">-{formatTokens(p.totalLost)}</td>
                      <td className="px-4 py-3">
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center gap-2">
              <User size={16} className="text-[#ff1fa6]" />
            ...
            {detailLoading ? (
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-full bg-[#ff1fa6]/20 flex items-center justify-center text-[#ff1fa6] text-xl font-black">
                    {selected.username[0]}
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { label: 'Balance', value: `🪙 ${formatTokens(selected.balance)}`, color: 'text-[#ffd700]' },
                    { label: 'Total Won', value: `+${formatTokens(selected.totalWon)}`, color: 'text-[#00e676]' },
                    { label: 'Total Lost', value: `-${formatTokens(selected.totalLost)}`, color: 'text-[#ff3d57]' },
                    { label: 'Total Bets', value: String(selected.totalBets || 0), color: 'text-white' },
                  ].map(({ label, value, color }) => (
                    <div key={label} className="bg-[#0a0010] rounded-lg p-2">
